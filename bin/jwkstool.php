...
use JWEDecoder\Keys\KeyException;
use JWEDecoder\Keys\KeyFactory;
use JWEDecoder\Keys\KeySet;
    ...
    protected function selectKey(InputInterface $input, OutputInterface $output)
    {
        if ($input->getArgument('kid')) {
            ...
            if ($input->getOption('use')) {
                $criteria['use'] = $input->getOption('use');
            }

            if ($input->getOption('op')) {
                $criteria['key_ops'] = explode(',', $input->getOption('op'));
            }
...
$app = new Application();
$app->setName('JWEDecoder JWKS tool');
